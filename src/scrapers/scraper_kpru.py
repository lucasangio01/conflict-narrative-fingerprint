...
class KyivPost:
    def __init__(self):
        self.start_url = "https://www.kp.ru/daily/euromaidan/"
    ...
    async def extract_data(self, session):
        ...
        urls = [urljoin(self.base_url, a.find("a")["href"]) for a in soup.find_all("div", attrs = {"class": "sc-1tputnk-12"})]
    ...
    async def extract_text(self, session):
        ...
        for soup in soups:
            ...
            title = soup.find("h1", attrs = {"class": "sc-j7em19-3 eyeguj"}).text.strip()
            all_texts.append(" ".join(paragraphs))
            all_titles.append(title)

        self.df_text = pd.DataFrame({"title": all_titles, "text": all_texts})
        self.df_text.to_csv("../../data/russia/kpru.csv")
