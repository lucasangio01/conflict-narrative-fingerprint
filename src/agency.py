...
IZ_PA_BASE = {
    ...
    # Israeli military/security — named institutions only
...
VIOLENT_VERBS = {"attack", "bomb", "kill", "destroy", "invade", "strike", "shell", "target", "raid", "fire", "shoot", "assassinate", "massacre", "wound", "besiege", "execute", "detain", "arrest", "expel"}
...
def resolve_token(token, doc):
    ...
    if token.dep_ in SUBJECT_OBJECT_DEPS:
        lemma = token.lemma_.lower()
        if lemma in PRONOUN_GROUPS:
            return lemma, PRONOUN_GROUPS[lemma], True
...
def calculate_uncertainty(sent):
    ...
    if len(sent) == 0:
        return 0.0
    count = sum(1 for t in sent if t.lemma_.lower() in MODAL_VERBS or t.lemma_.lower() in HEDGES)
...
for doc in tqdm(nlp.pipe(texts_list, batch_size=32, disable=["ner"]), total=len(texts_list)):
    for sent in doc.sents:
        ...
        for token in sent:
            ...
            if token.dep_ not in SUBJECT_OBJECT_DEPS:
                continue
